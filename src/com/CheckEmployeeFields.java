...
public class CheckEmployeeFields implements UniqueUsername{
    private String[] jobs = {"Librarian", "Receptionist", "Floor Keeper", "Books Dealer"};
    public void check(String name, String username, String password, String gender, String age, String job, String phone_number, String email_address){
        boolean username_check = false;
        boolean password_check = false;
        boolean name_check = false;
        boolean age_check = true;
        boolean job_check = false;
        ...
        if (username.length()>19 && username.length()<31){
            if (username.indexOf("@employee.library")>4){
        ...
        if (name_check && username_check && password_check && age_check && job_check && phone_number_check && email_check){
            CreateEmployee createEmployee = new CreateEmployee();
            createEmployee.add_employee_to_database(username, password, name, gender, Integer.parseInt(age), job, phone_number, email_address);
        }else{
            new IncorrectFields();
        }
    }
